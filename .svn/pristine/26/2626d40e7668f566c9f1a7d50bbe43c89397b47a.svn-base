...
public class Paging {
	 ...
	 * @return
	 */
	public static String getPagingFunction(int currentPage, int itemCount, int itemCountPerPage, int pageCountPerPaging,String funcName,String formName) {
		String paging = "";
		int maxPage = getMaxPage(itemCount, itemCountPerPage);
		List pagingList = getPagingList(currentPage, maxPage, pageCountPerPaging);
		...
		paging += "					<div class=\"pagelist\">" + "\n";
		paging += "						<input type=\"hidden\" name=\"currentPage\" value=\""+ currentPage +"\" />" + "\n";
		paging += "						<button type=\"button\" class=\"round gray\" onclick=\""+funcName+"("+formName+", 1);\">&nbsp;&lt;&lt;&nbsp;</button>" + "\n";
		paging += "" + "\n";
		if (currentPage > 1) {
			paging += "						<button type=\"button\" class=\"round gray\" onclick=\""+funcName+"("+formName+", "+ (currentPage-1) +");\">&nbsp;&lt;&nbsp;</button>" + "\n";
		}
		if (currentPage <= 1 ) {
			paging += "						<button type=\"button\" class=\"round gray\">&nbsp;&lt;&nbsp;</button>" + "\n";
		}
		...
		for (int i=0; i<pagingList.size(); i++) {
			if (Integer.parseInt(pagingList.get(i).toString()) == currentPage) {
				...
				paging += "								<button type=\"button\" class=\"round white\" onclick=\""+funcName+"("+formName+", "+pagingList.get(i)+");\">&nbsp;"+pagingList.get(i)+"&nbsp;</button>" + "\n";
			}
		}
		paging += "" + "\n";
		if (currentPage < maxPage) {
		...
		paging += "" + "\n";
		paging += "						<button type=\"button\" class=\"round gray\" onclick=\""+funcName+"("+formName+", "+ maxPage +");\">&nbsp;&gt;&gt;&nbsp;</button>" + "\n";
		paging += "						<div class=\"clear\"></div>" + "\n";
		paging += "					</div>";
