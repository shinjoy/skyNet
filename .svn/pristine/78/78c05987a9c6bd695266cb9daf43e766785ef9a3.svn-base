...
public class BbsComment {
	...
	public String getrAnswerStatusTxt() {
		String txt="";
		if(rAnswerStatus==Bbs.BBS_STATUS_NEW){
			txt="대기";
		}
		if(rAnswerStatus==Bbs.BBS_STATUS){
			txt="접수";
		}
		if(rAnswerStatus==Bbs.BBS_STATUS_ING){
			txt="진행";
		}
		if(rAnswerStatus==Bbs.BBS_STATUS_DROP){
			txt="보류";
		}
		if(rAnswerStatus==Bbs.BBS_STATUS_FINISH){
			txt="완료";
		}
		return txt;
	}
