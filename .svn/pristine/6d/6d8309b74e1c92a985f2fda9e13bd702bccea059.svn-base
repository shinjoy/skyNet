var projectAdmin = {
			...
			success:function(json) {
				...
				if (list.length > 0) {
					...
					for(var i=0;i<list.length;i++){
						...
						str+= ' 	<td class="text-left"> '+list[i].progressPercent + '</td>';
		...
		$.ajax({
			...
			success:function(json) {
				...
				if(project.projectFileName!=''){
					str +=' <div class="photo-detail" style="background-image:url('+project.projectFileName+');"></div>';
					
					str += '	<button type="button" class="del-btn" onclick="projectAdmin.fileDelete(\''+ project.projectFileName+'\',1)">X</button>';
				
					
					$("#img_1").append(str);
					$("#fileName").val(project.projectFileName);
					$("#file-input").css("display","none");
				}

				// 프로젝트명
				$("#project-title").html(project.projectName);
	...
	renderWorkerList : function(list, userlist) {
		...
		if(list.length>0){
			...
			//	workerList+='	<input type="hidden" name="projectSeq" value="'+list[i].projectSeq+'" > ';
				...
				workerList+='				<option value="PC" ' +(list[i].projectAdminPart=="PC" ? 'selected="selected"' : '')+'> PC </option> ';
				workerList+='			</select>';
				workerList+='		</td> ';
				workerList+='		<td> ';
	...
	renderScheduleList : function(project, todolist, paging,list) {
		...
		if(todolist.length>0){
			for(var i=0;i<todolist.length;i++){
				...
				scheduleList+=' 			<option value="PC" ' +(todolist[i].todoPart=="PC" ? 'selected="selected"' : '')+'> PC </option> ';
				scheduleList+='			</select>';
				scheduleList+=' 	 </td> ';
				scheduleList+=' 	<td> ';
	...
	projectEditDo : function(frm){
		var param = {
			 projectFileName : frm.fileName.value,	
			 companySeq : frm.companySeq.value,
			 projectSeq : frm.projectSeq.value,
	...
	formSuccess : function (responseText, statusText, xhr, $form) {
		...
		try {
			...
			if(json.photo!=""){
				$("#img_1").empty();
				var str = '';
				var fileName='/files'+json.path +'/'+ json.fileName;
				str +=' <div class="photo-detail" style="background-image:url('+fileName+');"></div>';
				
				str += '	<button type="button" class="del-btn" onclick="projectAdmin.fileDelete(\''+ json.path +'/'+ json.fileName +'\',1)">X</button>';
			
				
				$("#img_1").append(str);
				$("#fileName").val('/files'+json.path +'/'+ json.fileName);
				$("#file-input").css("display","none");
	...
	fileDelete : function(fileName,num){
