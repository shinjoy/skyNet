...
*/			projectSeq:frm.projectSeq.value,
		    ...
		    success:function(json) {
				...
				if (list.length > 0) {
					for(var i=0;i<list.length;i++){
						...
						str+='		<dd> ';
						str+='			<p class="contents" onclick="link.go(\'/m/admin_bbs_view.go?projectSeq='+frm.projectSeq.value+'&bbsSeq='+list[i].bbsSeq+'&page=1\');">';
					...
					if((parseInt(json.PERPAGE)*parseInt(json.currentPage))<parseInt(json.count)){
						...
						strpaging+=' <button type="button" onclick="adminBbs.getList(searchForm,'+(parseInt(json.currentPage)+1)+');">더보기 </button>';
						strpaging+=' </div>';
					}
		...
		$.ajax({
		    ...
		    success:function(json) {
				...
				if(bbs.userId==userForm.userId.value){
					...
					str+='		<button type="button" class="bbs-btn" onclick="adminBbs.goBbsEdit(pageForm,'+bbs.bbsSeq+',\'m\');">수정</button> ';
					str+='		<button type="button" class="bbs-btn" onclick="adminBbs.onBbsDelete('+bbs.bbsSeq+',0,pageForm,\'m\');">삭제</button> ';
					str+='	</div> ';
				}
				/*str+='	<div class="bbs-menu"> ';
				str+='		<img src="/images/ic_3spot_etc.png" onclick="adminBbs.showContextMenu('+bbs.bbsSeq+',event);">';
				str+='	</div> ';*/
				str+='	</dd> ';
		        ...
		        if(commentlist.length>0){
		        	for(var i=0;i<commentlist.length;i++){
		        		...
		        		if(commentlist[i].userId==userForm.userId.value){
							...
							commentstr+='		<button type="button" class="bbs-btn" onclick="adminBbs.onBbsDelete('+bbs.bbsSeq+','+commentlist[i].bbsCommentSeq+',bbsEditForm,\'m\');">삭제</button> ';
			        		commentstr+='	</div> ';
		        		}
		        		commentstr+='	 </dd> ';
		        		commentstr+='	</dl> ';
	...
	goBbsEdit : function(frm, seq,url) {
		...
		document.location.href = '/'+url+'/admin_bbs_edit.go?projectSeq='+frm.projectSeq.value+"&bbsSeq="+seq;
	},
		    ...
		    success:function(json) {
				...
				if(fileList != undefined){
					for(var i =0;i<fileList.length;i++){
						...
						photostr+='		<button type="button" class="file-delete" onclick="adminBbs.fileDelete('+fileList[i].bbsSeq+',\''+fileList[i].fileName+'\','+i+');">X</button><br> ';
						photostr+=' </p> ';
					}
				}
	...
	bbsSave : function(frm,url) {
		...
		$.ajax({
		    ...
		    success:function(json) {
		    	alert(json.message);
		    	if(json.result){
		    		document.location.href = "/"+url+"/admin_bbs_list.go?projectSeq="+frm.projectSeq.value;
		    	}
		    },
		    error:function(xhr, status, error) {
				console.log("err:",xhr, status, error);
	...
	bbsCommentedit : function(frm) {
		...
		$.ajax({
		    type:"POST",
		    url:"/proc/admin_bbs_comment_edit_do.go",
			data:param,
		    dataType:"json",
	...
	formSuccess : function (responseText, statusText, xhr, $form) {
		...
		try {
			...
			if(json.photo!=""){
				...
				str += '<li id="img_'+idx+'">';
				str += '	'+ json.fileName +' ';
				str += '		<button type="button" class="del-btn" onclick="adminBbs.fileDelete('+bbsEditForm.bbsSeq.value+',\''+ json.path +'/'+ json.fileName +'\','+idx+')">X</button>';
				str += '	</div>';
	...
	onBbsDelete : function(bbsSeq,bbsCommentSeq,frm,url) {
		if(confirm("삭제하시겠습니까?")) {
			...
			$.ajax({
				type:"POST",
				url:"/proc/admin_bbs_delete.go",
				dataType:"json",
				data:param,
				success:function(json){
					alert(json.message);
					if(bbsCommentSeq>0){
						document.location.href="/"+url+"/admin_bbs_view.go?projectSeq="+frm.projectSeq.value+"&bbsSeq="+frm.bbsSeq.value;
					}else{
						document.location.href="/"+url+"/admin_bbs_list.go?projectSeq="+frm.projectSeq.value;
					}
...
/*	
	...
	showContextMenu : function(key, event) {
	    ...
	    str += '	<li onclick="adminBbs.goBbsEdit(pageForm,'+key+');">수정</li>';
	    str += '	<li onclick="adminBbs.onBbsDelete(pageForm,'+key+');">삭제</li>';
	    str += '</ul>';
	    $("body").append(str);
	...
	showContextCommentMenu : function(key, event,contents) {
	    ...
	    var str = '';
	    str += '<ul class="context">';
	    str += '	<li onclick="adminBbs.commentSetContext(bbsEditForm,'+key+',\''+contents+'\');">수정</li>';
	    str += '	<li onclick="adminBbs.onBbsDelete(bbsEditForm);">삭제</li>';
	    str += '</ul>';
	    $("body").append(str);
