...
public class ProjectDao {
	...
	private RowMapper VprojectMapper = new RowMapper() {
		public Project mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			project.setPcPercent(Integer.parseInt(rs.getString("pc_percent")));
			return project;
		}
	};
	public int addProject(final Project project) {
		...
		query = "" +
				" select max(project_seq) from  T_NF_PROJECT ";
		return this.jdbcTemplate.queryForInt(query);
	}

	public void deleteProject(int project_seq) {
	...
	public List<Project> getProjectListNotPaging(int companySeq,String userId,int userLevel) {
		String con="where 1=1";
		
		if(companySeq>0){
			con+=" and company_seq = "+companySeq;
		}
		if(!userId.equals("")&&userLevel!=1){
			con+=" and project_seq in ( select project_seq from v_nf_project_admin where user_id='"+userId+"' )";
		}
